...
use dice_protocols::focusrite::spro24::*;
...
pub struct SPro24Model {
    ...
    tcd22xx_ctl: Tcd22xxCtl<SPro24State>,
}
...
impl CtlModel<SndDice> for SPro24Model {
    ...
    fn read(&mut self, unit: &mut SndDice, elem_id: &ElemId, elem_value: &mut ElemValue)
        -> Result<bool, Error>
    {
        if self.common_ctl.read(unit, &self.proto, &self.sections, elem_id, elem_value, TIMEOUT_MS)? {
            Ok(true)
    ...
    fn write(&mut self, unit: &mut SndDice, elem_id: &ElemId, old: &ElemValue, new: &ElemValue)
        -> Result<bool, Error>
    {
        if self.common_ctl.write(unit, &self.proto, &self.sections, elem_id, old, new, TIMEOUT_MS)? {
            Ok(true)
...
impl NotifyModel<SndDice, u32> for SPro24Model {
    ...
    fn parse_notification(&mut self, unit: &mut SndDice, msg: &u32) -> Result<(), Error> {
        self.common_ctl.parse_notification(unit, &self.proto, &self.sections, *msg, TIMEOUT_MS)?;
    ...
    fn read_notified_elem(&mut self, _: &SndDice, elem_id: &ElemId, elem_value: &mut ElemValue)
        -> Result<bool, Error>
    {
        if self.common_ctl.read_notified_elem(elem_id, elem_value)? {
            Ok(true)
...
impl MeasureModel<SndDice> for SPro24Model {
    ...
    fn measure_states(&mut self, unit: &mut SndDice) -> Result<(), Error> {
        self.common_ctl.measure_states(unit, &self.proto, &self.sections, TIMEOUT_MS)?;
